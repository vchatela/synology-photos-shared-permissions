...
# This script fixes orphaned ownership from PhotoStation migration to SynologyPhotos
...
fix_orphaned_ownership() {
    ...
    log_info "=== FIXING ORPHANED FOLDERS (UID 138862) ==="
    ...
    echo "$orphaned_folders" | while read folder; do
        if [ -n "$folder" ]; then
            if [ "$dry_run" = "true" ]; then
                ...
                if fix_folder_ownership "$folder" "orphaned UID 138862"; then
                    ((fixed_count++))
                else
                    ((failed_count++))
                fi
...
main() {
    ...
    case "$action" in
        ...
        *)
            ...
            echo "  fix-orphaned - Fix only orphaned UID 138862 folders"
